/***************************************************************************
                          computerplayer3_2.cpp  -  description
                             -------------------
...
ComputerPlayerBase::handIterator ComputerPlayer3_2::playAssist()
{
        ...
        if (push() == Card::hearts)
        {
                ...
                while (res != handEnd() and res->value() < highestOnTable.value()) ++res;
                --res;
                return res;
        }
        ...
        if (res->value() > highestOnTable.value()) return res;
...
/**  */
ComputerPlayerBase::handIterator ComputerPlayer3_2::playNotAssist()
{
        if (handEnd() - handBegin() == 13)
        {
                ...
                if (suitHasIt(Card::diamonds)) return highestOf(Card::diamonds);
                else if (suitHasIt(Card::spades))
                {
                        handIterator spade = highestOf(Card::spades);
                        if (spade->value() == Card::queen)
                        {
                                if (spade == std::find_if(handBegin(),handEnd(),OfSuite(Card::spades)))
                                {       // it is the lowest spade, besides this there are only hearts.
                                        return spade;
                                }
                                else if (spade != handBegin() and (spade - 1)->suit() == Card::spades) return spade -1;
                                else
                                {
                                        LOG_PLACE() << "OOPS.\n";
                                        massert(0);
        ...
        if (highest->value() > Card::eight or !suitHasIt(Card::hearts))
        {
                return highest;
        }
        return highestOf(Card::hearts);
...
/**  */
ComputerPlayerBase::handIterator ComputerPlayer3_2::giveOne()
{
        if (hasQueen()) return std::find(handBegin(),handEnd(),Cards::spadesQueen);
        if (hasHighSpades())
        {
...
ComputerPlayerBase::handIterator ComputerPlayer3_2::playStart()
{
        if (std::find_if(handBegin(),handEnd(),not1(OfSuite(Card::hearts))) == handEnd())
...
/**  */
const char* ComputerPlayer3_2::type() const{
        return "ComputerPlayer3_2";
}
