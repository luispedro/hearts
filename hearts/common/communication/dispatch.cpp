...
void Dispatch::points_handler(Message m)
{
		...
		if (!in) bad_message(m);
		else points_message(self,rigth,front,left);
}
...
void Dispatch::opponentname_handler(Message m)
{
		...
		if (!in) bad_message(m);
		else opponentname_message(player,name);
}	
