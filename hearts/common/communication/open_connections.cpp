...
int open_client_local_connection( const char* address )
{
	...
	if ( connect( fd, reinterpret_cast<sockaddr*>( &addr ), sizeof( addr ) ) < 0 ) {
		if ( errno == EINTR )
			return -connection_errors::interrupted;
...
{
	if ( address[ 0 ] == '/' )
		return open_client_local_connection( address );
	else
		return open_client_tcp_connection( address );
}

int open_client_connection( const char* address, short port )
{
	return open_client_tcp_connection( address, port );
}
