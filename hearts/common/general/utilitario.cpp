...
int Utility::Bind(int fd, void* addr, socklen_t len)
{
	...
	if (stat < 0) exit(1);//	throw UnixError("bind");
	return stat;
}
...
int Utility::Recvfrom(int fd, void* buf, size_t len,int flags, void* addr, socklen_t* slen)
{
	...
	if (stat < 0) exit(1); // throw UnixError("recvfrom");
	return stat;
}
...
int Utility::Socket(int a, int b, int c)
{
	...
	if (stat < 0) exit(1); // throw UnixError("socket");
	return stat;
}

void Utility::Connect(int fd, void* add,socklen_t len)
{
	if (connect(fd,(sockaddr*)add,len) < 0) exit(1); //throw UnixError("connect");
}
...
{
	int stat = listen(a,b);
	if (stat < 0) exit(1); // throw UnixError("listen");
}
...
void Utility::Sendto(int fd,const void* buf,int len,int f, void* addr,socklen_t addlen)
{
	...
	if (stat < 0) exit(1); // throw UnixError("sendto");
}
