...
{
	while (to_move->hand.count(Cards::clubsTwo) == 0) ++to_move;
	to_move->player->play();
...

{
	...
	table.reset();
	{
			...
			for (int i = 0; i != 4; ++i) (to_move + i)->player->currentStatus(hand_over[i]);
	}
	if (to_move->hand.empty())
	{ // checking this player is as good as checking any other
		assign_points();
		reset_game_data();
...
namespace { 
	...
	{
		if (c == Cards::spadesQueen) return 13;
		if (c.suit() == Card::hearts) return 1;
		return 0;
	}
}
...
// This is why this must be done in two parts: collect & distribute 
{
	...
	for (unsigned i = 0; i != 4; ++i)
	{
		for (std::set<Card>::iterator iter = players[i].gotten.begin(); iter != players[i].gotten.end(); ++iter)
		{
	...
	if (done_all != points_gotten.end())
	{
			...
			for (unsigned i = 0; i != 4; ++i)
			{
					if (i != done_all_index) players[i].points += 26;
			}
	} else {
		for (unsigned i = 0; i != 4; ++i)
		{
